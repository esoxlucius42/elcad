...
int countTriangles(const TopoDS_Shape& shape, double deflection)
{
#ifdef ELCAD_HAVE_OCCT
    BRepMesh_IncrementalMesh mesh(shape, static_cast<float>(deflection), Standard_False, 0.5, Standard_True);
    int total = 0;
...
int sphere_tess_count_main(int argc, char** argv)
{
    ...
    for (double d : defs) {
        int tris = countTriangles(sphere, d);
        std::cout << "deflection=" << d << " -> triangles=" << tris << std::endl;
    }
#else
